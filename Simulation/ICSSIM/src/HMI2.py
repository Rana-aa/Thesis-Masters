...
class HMI2(HMI):
    ...
    def _display(self):
        ...
        menu += self.__get_menu_line(3, 'pinns on/off')
    ...
    def _operate(self):
        try:
            ...
            if input1 == 1:
                ...
                self._send(TAG.TAG_PINNS_STATUS, input2)
    ...
    def __get_choice(self):
        ...
        if input1 == 3:
            sub_menu = '\n'
            sub_menu += "0) Send command for off\n"
            sub_menu += "1) Send command for on\n"
            self.report(sub_menu)
            input2 = int(input('Command (1 to 1): '))
            if input2 < 0 or input2 > 1:
                raise ValueError('Just 0, and 1 are acceptable for command')
        else:
            sub_menu = '\n'
