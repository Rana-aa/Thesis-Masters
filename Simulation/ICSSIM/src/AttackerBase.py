...
from ics_sim.Attacks import _do_scan_scapy_attack, _do_replay_scapy_attack, _do_mitm_scapy_attack, \
    _do_scan_nmap_attack, _do_command_injection_attack, _do_ddos_attack, _do_ping_sweep_attack


class AttackerBase(Runnable, ABC):

    NAME_ATTACK_SCAN_MMAP = 'scan-nmap'
    NAME_ATTACK_SCAN_PING = 'scan-ping'
    ...
    def __init__(self, name1):
        ...
        self.attack_list = {
            ...
            AttackerBase.NAME_ATTACK_SCAN_PING: 'ping-scan',
    ...
    def _apply_attack(self, name):
        ...
        elif name == AttackerBase.NAME_ATTACK_SCAN_PING:
            self._scan_ping_attack()
    ...
    def _replay_scapy_attack(self, timeout=15, replay_count=3, target='192.168.0.11,192.168.0.21'): # changed to 21 intead of 22
    ...
    def _scan_ping_attack(self):
        name = AttackerBase.NAME_ATTACK_SCAN_PING
        log_file = os.path.join(self.log_path, f'log-{name}.txt')
        start = datetime.now()

        _do_ping_sweep_attack(log_dir=self.log_path, log_file=log_file)
    ...
    def _ddos_attack(self, ddos_agent_path='DDosAgent.py', timeout=60, num_process=10, target='192.168.0.12'):
